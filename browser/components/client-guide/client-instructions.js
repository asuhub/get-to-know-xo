const clientInstructions = [
  ...
  '4. To make a PUT request to /people and modify the attribute city to be “Brooklyn”: if you are still on the page from step 3 (viewing the recently added user from the __add new__ page) simply click on the pencil icon of the user listed on your screen and change either the name or city, then hit save. OR you can navigate to either the __view all__ tab (will make another request to GET /people) and click on the pencil next to that user, or __view by id__ tab, search for the ID of the person (will make a request to GET /people/:personId) and then click on the pencil icon to the right of the user that appears, then edit from there. A modal will pop up where you can edit the favorite city of that user (and name, if you so choose).',
